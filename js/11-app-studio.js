...
function renderStudioSysTabs() {
    ...
    if (typeof TREATMENT_LIB === 'undefined' || !TREATMENT_LIB) {
        el.innerHTML = '<div class="tst-empty">⚠️ TREATMENT_LIB missing (js/10-treatment-data.js)</div>';
        return;
    }
...
function renderStudioList() {
    ...
    if (typeof TREATMENT_LIB === 'undefined' || !TREATMENT_LIB) {
        el.innerHTML = '<div class="tst-empty">⚠️ js/10-treatment-data.js لوڈ نہیں ہوئی</div>';
        return;
    }
    ...
    if (studioTab === 'sym') {
        ...
        function studioKwGroup(kind, title, always) {
            ...
            var body = chips.length
                ...
                : '<div class="tst-poolhint" style="margin:4px 0 8px">' + ({ ur: 'اس بیماری کی متعلقہ ادویات میں الگ concomitant درج نہیں', en: 'No separate concomitant listed for this disease remedies', roman: 'Is bimari ki adviat mein alag concomitant nahi' }[L]) + '</div>';
...
function renderStudioAll() {
    try { renderStudioSysTabs(); } catch (e) { console.error(e); }
    try { renderStudioList(); } catch (e) { console.error(e); }
    try { renderStudioDetail(); } catch (e) { console.error(e); }
}
window.renderStudioAll = renderStudioAll;
...
function studioBoot() {
    studioBindViewButtons();
    var q = document.getElementById('studioSearch');
    if (q && q.getAttribute('data-bound') !== '1') {
        q.setAttribute('data-bound', '1');
        q.addEventListener('input', renderStudioList);
    }
    var pq = document.getElementById('studioPatientQuery');
    if (pq && pq.getAttribute('data-bound') !== '1') {
        pq.setAttribute('data-bound', '1');
        pq.addEventListener('input', studioSearchPatients);
    }
    var saved = null;
...
document.addEventListener('DOMContentLoaded', studioBoot);
window.addEventListener('load', function() {
    var list = document.getElementById('studioList');
    if (list && !list.innerHTML.trim()) studioBoot();
});
