...
## v82 — Windows Screen Parser (English/OFFLINE) — 26 Sep 2026
- **New standalone desktop companion (not the v81 in-browser paste parser):** `tools/screen_parser/{app,ocr,engine,store}.py`, `__main__.py`, README_UR.md; Windows BAT installer/launcher and optional on-Windows PyInstaller builder at repo root. Requires Python 3.11–3.13 + local Tesseract `eng`; `Pillow,mss,pytesseract,PyMuPDF` via separate `tools/screen_parser/requirements.txt`. No cloud calls. GUI is Tk: small always-on-top control bar selecting a region on another visible app; stable-scroll auto-frame capture + separate one-page-at-a-time check/review; optional viewer-counter OCR for auto boundary (manual Check page fallback). Single/two columns within ONE physical page; screenshot-file cropping and optional original PDF OCR/text layer. Frozen full-screen review keeps source screenshot left, zoom to flagged line, editable OCR, per-entry path/grade/narrative fields and advanced JSON on right. Changed full-page OCR retries capped at 2 across sessions, targeted line retry separately, recapture or manual edit, individual false-alarm/corrected/verified resolutions with reason, no next page until approved/explicitly skipped.
- **Durable ledger:** `Ledger` SQLite in `%LOCALAPPDATA%\BismillahClinic\ScreenParser\` (user machine, not repo). Book title/edition/total physical PDF pages/selected first–last content pages/source fingerprint/verified colour palette; unique `(book_id,physical_page)` row; captures with image hash; raw + corrected text and parsed structures; session/day unique checked and first-approved counts; pending next page; separate printed folio; events/audit and autosaved editor drafts. No silent jump/duplicate. Resolving a false alarm cannot fill an unknown remedy grade; approval/export still require valid grade and path. Low confidence, independent OCR disagreements (including local word/parenthesised remedy changes), missing scroll overlap, viewer counter/total changes and structural/count/unmapped issues are warnings. **Automatic pass is only machine checks, NOT certainty about image.** Source images are NOT embedded in export.
- **Three conservative book profiles:** `repertory` prints `{rid:{t,r}}` chapter+index/master; `materia_medica` prints `{remedies:{abbr:{name,sections:[{h,p}]}},unmatched}`; `prescriber` emits BOTH clinic-compatible library `{sections:[{h,p}]}` and `clinical_index` containing only explicit remedy/See references (NO invented grading/therapy). Unknown colours/grades are `None` until a book-specific source legend is verified or a human enters grades. `remedy_names.json` is READ ONLY. Physical page and printed page number are different (Quick PDF 24/750 = printed 1); progress is keyed by physical page. Long-book complete export is blocked until every selected physical page is approved or explicitly skipped; ZIP contents under `staging/`, with manifest + page approval kind and screenshot SHA. **Do not overwrite live data JSON; this staging ZIP does NOT self-install into the clinic. Clinic importer/search are separate future work.** Source rights/privacy: no user screenshots/PDFs committed or uploaded to cloud.
- **Tests/evidence:** `python3 tests/test_screen_parser_v82.py -v` → 19 PASS; existing 10 JS regression suites PASS/10 on full upstream clone with index v82/SW v96 overlaid. Includes real local Tesseract on generated image + PyMuPDF synthetic PDF, SQLite/ZIP/export, resumability/false-alarms/grades. Tk toolbar/review Xvfb smoke passes in Linux; end-to-end virtual desktop mss screenshot → async English OCR → page approval of generated Abies UI also passes. User screenshots processed *locally only* for feasibility: Allen Abies + 19 sections and local OCR disagreement caught; full image → human fix → approved → MM staging export smoke passed; tiny two-col Synthesis incomplete and flagged; Prescriber clipped crop flagged. Native Windows permissions/display/installer/EXE not executable in Linux sandbox → **real Windows acceptance test still outstanding**. Full instructions and caveats in `tools/screen_parser/README_UR.md` / `UPDATE_NOTES_v82.txt`.
- **Cache standing rule:** index parser-related `?v=82`, SW `CACHE_NAME='bhc-clinic-v96'`; no existing JSON data changed. User pushes manually with ZIP over repo root, never request credentials.
